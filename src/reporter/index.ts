...
import type { AuditReport, BaselineDiff, BatchAuditReport, OutputFormat } from '../types.js';

export function report(auditReport: AuditReport, format: OutputFormat, diff?: BaselineDiff): void {
  switch (format) {
    case 'json':
      reportJson(auditReport, diff);
      break;
    case 'html':
